...
#pragma omp parallel for private(j, k, compute_it) reduction(+:aggregate) collapse(3)
  for (i=1; i<DIM-1; i++) {
    for (j=1; j<DIM-1; j++) {
      for (k=1; k<DIM-1; k++) {
...
#pragma omp parallel for private(j, k, u, aggregate) collapse(3)
  for (i=1; i<DIM-1; i++) {
    for (j=1; j<DIM-1; j++) {
      for (k=1; k<DIM-1; k++) {
        ...
        aggregate = 0;
        for (u=-1; u<=1; u++) {
          int temp1 = (i+u)*DIM*DIM;
          
          int temp2 = (j-1)*DIM;
          aggregate += old[temp1+temp2+(k-1)];
          aggregate += old[temp1+temp2+(k)];
          aggregate += old[temp1+temp2+(k+1)];
        ...
        aggregate /= 27;
        new[i*DIM*DIM + j*DIM + k]= aggregate;
      }
    }
  }
